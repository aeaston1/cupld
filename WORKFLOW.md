...
agent:
  ...
  max_turns: 10
codex:
  command: codex --config shell_environment_policy.inherit=all --config 'model="gpt-5.5"' --config model_reasoning_effort=xhigh --config 'service_tier="fast"' app-server
...
10. Prefer targeted Rust validation. Use `cargo test` when the change affects behavior broadly, and narrower package/test commands when appropriate.
...
3. Create or reuse a branch for this issue:
...
4. Implement the ticket with focused commits.
5. Run relevant validation before publishing. For broad behavior changes, run `cargo test`.
6. If no code changes are required, update Linear with a concise note and move the issue to `Human Review`.
